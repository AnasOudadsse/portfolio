...
const projectsData = [
  {
    ...
    description: {
      en: "RevOps dashboard powered by n8n automations for the CEO of a Commercial Real Estate Education company. Automatically orchestrates data between Airtable (used as a headless backend across 9 tables: Contacts, Sales, Zoom Attendance, Surveys, Action Logs, etc.), Zoom, and the marketing stack. Built as a secure full-stack Next.js app with server-side API Route Handlers to hide Airtable and third-party tokens, plus ISR caching (5-minute refresh) to stay within Airtable’s 5 req/s limit while keeping KPIs fresh. Zoom webhooks are deduped via event_ts, and UTM parameters, watch time, and sales deposits are merged to compute real-time outstanding pipeline and lead quality scores. Delivered as a minimalist multi-tab executive dashboard with Recharts (sales velocity, pipeline quality donuts, funnel drop-off bars).",
      fr: "Tableau de bord RevOps piloté par des automatisations n8n pour le CEO d'une entreprise d'éducation en immobilier commercial. Orchestration automatique des données entre Airtable (backend headless connecté à 9 tables : Contacts, Ventes, Présence Zoom, Sondages, Journaux d’actions, etc.), Zoom et la stack marketing. Développé comme une application full-stack sécurisée avec Next.js : API Route Handlers côté serveur pour cacher les tokens Airtable et tiers, et mise en cache ISR (rafraîchissement toutes les 5 minutes) pour respecter la limite de 5 req/s tout en gardant les KPI à jour. Les webhooks Zoom sont dédupliqués via event_ts et les paramètres UTM, temps de visionnage et dépôts de vente sont fusionnés pour calculer en temps réel le pipeline restant et les scores de qualité des leads. L’interface est un tableau de bord exécutif multi-onglets minimaliste avec Recharts (vélocité des ventes, donuts de qualité du pipeline, barres de décrochage d’entonnoir).",
    },
    imageSrc: "/3DE-projetc.png",
    tags: [
      "Next.js",
      "React",
      "Tailwind CSS",
